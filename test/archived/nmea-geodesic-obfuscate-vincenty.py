#!/usr/bin/env python3
"""
nmea-geodesic-obfuscate-vincenty.py
...
This script implements direct and inverse geodesic calculations using a
self-contained Vincenty formula (inverse and direct) fully compatible with
the WGS84 ellipsoid. No external dependencies are required.

Usage:
    python3 nmea-geodesic-obfuscate-vincenty.py infile.txt outfile.txt --house LAT LON [--village-center LAT LON] [--preview N]

Example:
    python3 nmea-geodesic-obfuscate-vincenty.py nmea-in.txt nmea-out.txt --house 44.138821609323024 -72.6421729447827 --preview 5

Notes:
- House coordinates are required (private).
- Village center defaults to (44.148630208502965, -72.65679699901393).
- Vincenty inverse/direct geodesics, based on original 1975 formulas.
- Self-contained; no external dependencies.

Credits:
- Geodesic algorithm: Vincenty, 1975
...
# ---------- Geodesic inverse/direct (as before) ----------
...
def dm_to_deg_safe(dm_str: str, hemi: str):
    if not dm_str or not hemi:
        return None
    if hemi in ('N','S'):
        if not _RE_LAT.match(dm_str):
            return None
    else:
        if not _RE_LON.match(dm_str):
            return None
    try:
        v = float(dm_str)
    except ValueError:
...
# ---------- Sentence processing ----------
# correct indices:
# GGA: $--GGA, time(1), lat(2), NS(3), lon(4), EW(5), ...
# RMC: $--RMC, time(1), status(2), lat(3), NS(4), lon(5), EW(6), ...
# GLL: $--GLL, lat(1), NS(2), lon(3), EW(4), time(5), status(6), mode(7)
GN_POS_TYPES = {
    'GGA': (2, 3, 4, 5),
    'RMC': (3, 4, 5, 6),
    'GLL': (1, 2, 3, 4)
...
def process_line(line: str, distance_m: float, az_deg: float, last_valid: dict):
    ...
    if key in GN_POS_TYPES:
        ...
        # make sure parts list is long enough; if not, leave line unchanged
        ...
        parts[lat_i] = lat_field
        parts[lat_h_i] = lat_h
        parts[lon_i] = lon_field
        parts[lon_h_i] = lon_h
        body = ",".join(parts)
